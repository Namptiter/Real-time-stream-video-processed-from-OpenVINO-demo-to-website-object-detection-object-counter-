...
int main(int argc, char *argv[]) {
    ...
    int check;
    int old=0;
    unsigned char *a_counttt;
    int clientSocket,clientSocket1;
    struct sockaddr_in serverAddr;
    struct sockaddr_in serverAddr1;
    char s_addr[] = "127.0.0.1";
    char s_addr1[] = "127.0.0.2";
    
    clientSocket = socket(PF_INET, SOCK_STREAM, IPPROTO_TCP);
    clientSocket1 = socket(PF_INET, SOCK_STREAM, IPPROTO_TCP);
    ...
    serverAddr1.sin_addr.s_addr = inet_addr(s_addr1);
    // Set all bits of the padding field to 0
    memset(serverAddr.sin_zero, '\0', sizeof(serverAddr.sin_zero));
    connect(clientSocket, (struct sockaddr*)&serverAddr, sizeof(serverAddr));
    memset(serverAddr1.sin_zero, '\0', sizeof(serverAddr1.sin_zero));
    connect(clientSocket1, (struct sockaddr*)&serverAddr1, sizeof(serverAddr1));
	...
	std::cout << "Number of object " << check << std::endl;
    if(check==0)
    {
        a_counttt=(unsigned char*)"0";
    }
    else if(check==1)
    {
        a_counttt=(unsigned char*)"1";
    }
    else if(check==2)
    {
        a_counttt=(unsigned char*)"2";
    }
    else if(check==3)
    {
        a_counttt=(unsigned char*)"3";
    }
    else if(check==4)
    {
        a_counttt=(unsigned char*)"4";
    }
    else if(check==5)
    {
        a_counttt=(unsigned char*)"5";
    }
    else if(check==6)
    {
        a_counttt=(unsigned char*)"6";
    }
    else if(check==7)
    {
        a_counttt=(unsigned char*)"7";
    }
    else if(check==8)
    {
        a_counttt=(unsigned char*)"8";
    }
    else if(check==9)
    {
        a_counttt=(unsigned char*)"9";
    }
    std::cout  << "helo" << a_counttt << std::endl;
    send(clientSocket1, a_counttt, sizeof(a_counttt), 0);
	old=check;
}
                ...
                // out << "Count" << std::fixed << std::setprecision(2) << check;
                ...
                //     cv::putText(frame, out.str(), cv::Point2f(0, 75), cv::FONT_HERSHEY_TRIPLEX, 0.4,
                //                 cv::Scalar(255, 0, 0));
                // }
                ...
                for (auto &object : objects) {
                    ...
                    if (FLAGS_r) {
                        std::cout << "[" << label << "] element, prob = " << confidence << "count" << check <<
    ...
    send(clientSocket1, a_counttt, sizeof(a_counttt), 0);
                        ...
                        cv::putText(frame,
                                (label < static_cast<int>(labels.size()) ?
                                    ...
                                    cv::Point2f(static_cast<float>(object.xmin), static_cast<float>(object.ymin - 5)), cv::FONT_HERSHEY_COMPLEX_SMALL, 0.5,
                                    cv::Scalar(0, 255, 255));
                        cv::rectangle(frame, cv::Point2f(static_cast<float>(object.xmin), static_cast<float>(object.ymin)),
                                      cv::Point2f(static_cast<float>(object.xmax), static_cast<float>(object.ymax)), cv::Scalar(0, 255, 255));
                    }           
            old=check;
